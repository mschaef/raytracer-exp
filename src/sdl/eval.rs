...
// ---------------------------------------------------------------------------
//
// `when`, `when-not`, `cond`, `->` and `->>` used to live here as
// special forms. They are now source rewrites in the desugaring pass
// (`src/sdl/desugar.rs`), so the evaluator never sees them.
